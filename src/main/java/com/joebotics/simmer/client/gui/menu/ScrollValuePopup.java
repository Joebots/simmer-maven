...
public class ScrollValuePopup extends PopupPanel implements MouseOutHandler,
	...
	public ScrollValuePopup(final int x, final int y, int dy, AbstractCircuitElement e, Simmer s) {
		super();
		myElm = e;
		deltaY = 0;
		sim = s;
	...
	private void setupLabels() {
		...
		for (int i = 0; i < labMax; i++) {
			...
			if ((thissel + i - 2) < 0 || (thissel + i - 2) >= nvalues)
				...
				labels[i].setText(AbstractCircuitElement.getShortUnitText(values[thissel
						+ i - 2], ""));
	...
	private void setupValues() {
		...
		for (int i = 0; i < nvalues + 1; i++) {
			if (AbstractCircuitElement.getShortUnitText(currentvalue, "") == AbstractCircuitElement
					.getShortUnitText(values[i], "")) { // match to an existing
														// value
				values[i] = currentvalue; // Just in case it isn't 100%
											// identical
				currentidx = i;
				break;
			}
